...
1. 新增后端本地通知器 `backend/app/notifications/notifier.py`
2. 工作流开始执行、等待确认、完成和中断时会写入本地通知日志
3. Windows 环境下会尝试发送桌面气泡通知，并在失败时自动回退到日志
4. 可通过环境变量控制是否启用通知与声音提醒
5. 项目页已支持本地项目搜索、创建、编辑、删除与对话迁移
6. 项目页可直接打开指定项目对话，并保持当前项目上下文
7. 聊天页已支持当前对话内的关键词搜索、匹配高亮与结果跳转
8. 设置页已支持用户自定义 Agent 角色模板管理，并可参与工作流重新规划

当前下一步目标：

1. 继续增强日志面板与 Agent 配置导入导出体验
